...
```bash
npm install && ./build.sh     # → build/ и vot-button-v1.0.9.zip
```
...
Полупрозрачная кнопка **Translate** появляется у верхнего края плеера. Перетащи панель
за ручку `⋮⋮`, чтобы выбрать положение; оно сохраняется для следующих видео. Клик →
перевод (первый раз Яндекс может ставить видео в очередь, расширение само ждёт до
12 попыток). При ошибке наведи на **Retry**, чтобы увидеть причину, и нажми ещё раз.
Рядом — выбор голоса
